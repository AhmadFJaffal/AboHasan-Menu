...
const Menu = ({ items }) => {
  ...
  const formatPrice = (item) => {
    ...
    const perMealSuffix =
      item.kiloPrice && item.kiloPrice !== "NaN" ? " (Per Meal)" : "";

    // Check and format the meal price (regular price)
    if (!item.price || item.price === "NaN") {
      priceDisplay.push(
        <h4 className="price" key="meal-price">
          X LBP
        </h4>
      ); // Default display for invalid or missing prices
    } else {
      priceDisplay.push(
        <h4 className="price" key="meal-price">
          {item.price} LBP{perMealSuffix}
        </h4>
      );
    }
    ...
    if (item.kiloPrice && item.kiloPrice !== "NaN") {
      priceDisplay.push(
        <h4 className="price" key="kilo-price">
          {item.kiloPrice} LBP (Per Kilo)
        </h4>
      );
    }
